...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        >>> dist['f'] = 10
        >>> list(sorted(dist.items()))
        [('a', 0.2), ('b', 0.4), ('c', 0.4), ('d', 0.0), ('e', 4), ('f', 10)]
        ...
        # print(self)     # - Self is a dictionary
        # print("TOTAL", self.total())

        #Get total of dictionary
        totalDictVal = self.total()

        #normalize - set value per key as a percentage of total sum of all keys
        #if total == 0 do nothing
        if totalDictVal > 0:
            for key in self:
                # print(key)
                self[key] = self[key] / totalDictVal
                # print(self[key])
        "*** END YOUR CODE HERE ***"
    ...
    def sample(self):
        ...
        # randomize sample of uniform distribution [0,1) {x such that a≤x<b}=[a,b)
        randNum = random.random()
        # print(randNum)

        sortedDict = sorted(copyDict.items())
        # print(sortedDict)

        # Sampling of random distribution
        i = 0
        for key, probability in sorted(sortedDict):
            # print(key, probability + i, randNum)
            if i <= randNum < probability + i:
        ...
        raise Exception("ERROR: cannot get sample")
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        probability of noisyDistance given pacman's position and ghosts' positions
        """
        "*** YOUR CODE HERE ***"
        # raiseNotDefined()
        # print(ghostPosition, jailPosition)
        isGhostJailed = ghostPosition == jailPosition
        # print(noisyDistance)
        isObservationNone = noisyDistance == None

        # print(isGhostJailed, isObservationNone)
        #instances where ghost is jailed or observation is none - probability is either 1 or 0
        if isGhostJailed or isObservationNone:
            # print(ghostPosition, jailPosition, isGhostJailed, isObservationNone)
            #special case ghost is jailed and observation is 0
            #means pacman caught a ghost
            if isGhostJailed and isObservationNone:
                # print(1)
                return 1
            #ghost not in jail and observation is 0 - sensors not picking up ghost
            #ghost is in jail and observation is not 0 - ghost is already in jail no need to observe
            else:
                # print(0)
                return 0

        # use manhattan distance as instructed to get distance between pacman and ghost
        mDistance = manhattanDistance(pacmanPosition, ghostPosition)
        # print("NOISY DISTANCE", noisyDistance, "MANHATTAN DISTANCE", mDistance)

        # print(busters.getObservationProbability(noisyDistance, mDistance))
        # probability of noisy distance given the manhattan distance
        probFunc = busters.getObservationProbability(noisyDistance, mDistance)

        # Return predefined function to get P(noisyDistance | trueDistance)
        return probFunc
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # print(posPacman, posJail)

        # print(self.allPositions)
        # print(observation, posPacman, posJail)

        #get the probability P(observation | posPacman, posGhost, posJail)
        for posGhost in self.allPositions:
            probability = self.getObservationProb(observation, posPacman, posGhost, posJail)
            # print(observation, posPacman, posGhost, posJail, probability)

            #update belief
            # print(observation, posPacman, posGhost, posJail, self.beliefs[posGhost], probability)
            self.beliefs[posGhost] *= probability
            # print(observation, posPacman, posGhost, posJail, self.beliefs[posGhost], probability)
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        #Set temporary variable for belief
        beliefTemp = DiscreteDistribution()
        # print("UNUPDATED: ", beliefTemp)

        # print(self.allPositions) - same as for observe update method
        for oldPos in self.allPositions:
            # Get distribution of t+1 for ghosts given t=oldPos
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            # print(oldPos, newPosDist)

            #for each possible next move/next state of ghost calculate probabilities to infer next possible position
            for newPos, probability in newPosDist.items():
                # print(oldPos, self.beliefs[oldPos], newPos, beliefTemp[newPos], probability)
                beliefTemp[newPos] += self.beliefs[oldPos] * probability
                # print(beliefTemp[newPos])

                # update belief
                # self.beliefs[posGhost] *= probability

        #update self.beliefs
        # print("UPDATED: ", beliefTemp)
        self.beliefs = beliefTemp
